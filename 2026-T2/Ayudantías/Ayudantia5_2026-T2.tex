...
s_{b,t-1} &\le \mathit{Alm}_{b}\,(1 - c^{B}_{bt})
&& \forall b\in\mathcal{B},\ t\in\mathcal{T}
&& \text{(vaciar antes de cerrar)} \label{c:vaciar}\\[2pt]
...
\noindent En las transiciones \eqref{c:transp} y \eqref{c:transb}, el término $y_{p,0}$ (resp.\ $z_{b,0}$) es el \emph{estado inicial dado} de la instalación; y en el balance \eqref{c:balance} y en \eqref{c:vaciar}, $s_{b,0}$ es el \emph{inventario inicial dado}. Todos son parámetros, no variables (así, cerrar una bodega en $t=1$ exige que su inventario inicial $s_{b,0}$ sea nulo).
...
\textbf{Vaciar antes de cerrar.} La restricción \eqref{c:vaciar} exige que una bodega solo pueda cerrarse si llegó \emph{vacía} al final del periodo anterior. Su lógica es un big-M limpio: si $c^{B}_{bt}=1$ (se cierra en $t$), el lado derecho $\mathit{Alm}_b(1-c^{B}_{bt})$ colapsa a $0$ y fuerza $s_{b,t-1}=0$; si no se cierra, $c^{B}_{bt}=0$ y la cota se relaja a $\mathit{Alm}_b$, quedando redundante con \eqref{c:capbodega}. El big-M natural es, otra vez, la propia capacidad de almacenamiento. Sin esta restricción, el modelo podría \emph{cerrar de golpe} una bodega con stock y liquidarlo en el mismo periodo de cierre —una ``bodega inactiva que aún despacha''—, artificio que aquí eliminamos: el plan óptimo debe programar el vaciado \emph{mientras la instalación sigue operativa} (en $t-1$) y solo entonces apagarla, que es como ocurre el cierre de un centro de distribución en la realidad.
